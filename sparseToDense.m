...
% For cars4, sparseToDense('cars4', 0.2, 5, 5, 1.75, 1)
%%
...
%     video = 'cars1';
    ...
    % Output Video file
    mkdir(fullfile(rootDir, video, 'output'));
    framerate = 3;
...
%         lambda = 5;
    ...
    %     E = inf; Eafter = 0;
        ...
        if exist(gtFile, 'file')
            ...
            saveas(gcf, fullfile(rootDir, video, 'output', ['confusion', num2str(f+1), '.jpg']));
            fig = figure('Name', ['Frame ' num2str(f+1) ' - Results'], 'NumberTitle', 'off');
            set(fig, 'WindowStyle', 'docked');
            plotroc(gtK, maskK);
            saveas(gcf, fullfile(rootDir, video, 'output', ['roc', num2str(f+1), '.jpg']));
