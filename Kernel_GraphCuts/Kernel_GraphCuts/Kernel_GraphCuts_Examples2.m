function [logical_cut]=Kernel_GraphCuts_Examples(Imask,path,result,BW)
...
BW_adjust(min_row:max_col,min_col:max_col) = BW(min_row:max_col,min_col:max_col);
for i =1:k
cluster = img(:,:,1)==cluster_ids(i) & BW_adjust==1;    
if nnz(cluster)~=0
    count = count+1;
    cluster_id_labelled(count) = cluster_ids(i);
end
