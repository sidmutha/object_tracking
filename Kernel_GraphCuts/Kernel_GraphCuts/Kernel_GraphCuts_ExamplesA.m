...
cutou = zeros(size(cut_connected))
for l = 1:CC.NumObjects %// Loop through each object and plot it in white. This is where you can create individual figures for each object.
BW2 = zeros(size(cut_connected)); %// Create dummy image for display purposes.

    PixId = CC.PixelIdxList{l}; %// Just simpler to understand

    if size(PixId,1) ==1 %// If only one row, don't consider.        
        continue
    else
    BW2(PixId) = 1;
    number_fg=nnz(BW2==1 & BW_adjust==1)
      if number_fg>20
          offset=find(BW2==1);
          cutou(offset) = 1; 
