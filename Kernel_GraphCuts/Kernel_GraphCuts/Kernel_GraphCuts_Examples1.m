...
% path = 'N:\research\DATA\STABILIZATION\framesma_unstable\C\0051.jpg';%
...
im1(:,:,4) = 1*result(:,:,1);
im1(:,:,5) = 1*result(:,:,2);
[Gx1, Gy1] = gradient(double(im(:,:,1)));
[Gx2, Gy2] = gradient(double(im(:,:,2)));
[Gx3, Gy3] = gradient(double(im(:,:,3)));
sumGx = abs(Gx1)+abs(Gx2)+abs(Gx3);
sumGy = abs(Gy1)+abs(Gy2)+abs(Gy3);
Gx = Gx1 & Gx2 & Gx3;
Gy = Gy1 & Gy2 & Gy3;
G = Gx+Gy;
off = (G==2);
G1=zeros(size(G));
G1(off) = (sumGx(off)+sumGy(off));

im1(:,:,6) = 0*G1;
% Initializations: cluster the data into k regions
tic,
disp('Start kmeans');
% im1=result;
data = ToVector(im1);
...
min_col = min(col)+0;
max_col = max(col)-0;
% min_col = round((min(col)+max(col))/2-20);
% max_col = round((min(col)+max(col))/2+15);
min_row = min(row)+0;
max_row = max(row)-0;
...
for i =1:k
cluster = img(:,:,1)==cluster_ids(i) & Imask==1;%BW_adjust==1; 
no_of_fg_points_alloted_for_cluster(i) = nnz(cluster);
if nnz(cluster)~=0
    count = count+1;
...
%     offset(:,1) = rows;offset(:,2) = cols;
  ...
  CC= bwconncomp(cut_connected);
  mark=zeros(size(cut_connected,1),size(cut_connected,2));
  for j=1:CC.NumObjects
       l=zeros(size(cut_connected,1),size(cut_connected,2));
        l(CC.PixelIdxList{1,j})=1;
       nfg= nnz(l==1 & BW==1);
        sz=size(CC.PixelIdxList{1,j});
        if nfg==0
            mark=mark;
        elseif sz<1200
            mark(CC.PixelIdxList{1,j})=1;
       
        elseif nfg> 0.04*nnz(CC.PixelIdxList{1,j}) 
...
log_cut=sum(cutout,3)>=1;
logical_cut=(log_cut==1 & Imask1==1);
hold on
h=imshow(BW_adjust);
set(h,'AlphaData',0.5);
hold off
